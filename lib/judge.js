...
// 社名そうじマシン： "株式会社テレモ"・"てれも"・"ﾃﾚﾓ" → "テレモ"
// 方針：
//   1. NFKC正規化で「半角カナ→全角カナ」「全角英数→半角」などをそろえる
//   2. ひらがな→カタカナに変換（てれも と テレモ を同じ扱いに）
//   3. 会社の種類を表す言葉（株式会社・㈱など）と空白を消す
//   4. 英字は小文字にそろえる
// ※ ただし英字表記(TELEMO)とカナ表記(テレモ)は別物として残る。
//   だから運用ルールとして「企業名はカタカナで入力」に統一する。
